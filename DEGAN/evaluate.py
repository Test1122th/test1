...
def evaluate_D(post_process_dic, input_df_dic, window_length, model):
    ...
    input_df_dic["test_df"]['y_pred']=np.round(model.predict(input_df_dic["test_df"].iloc[:,1:(window_length+1)])[:,0])  # [0,1] taken as 0, [1,0] taken as 1
    ...
    try:
        ...
        peaks, _ = find_peaks(KDE_scores.flatten(),height=np.histogram(KDE_scores,bins=21)[1][5],distance=50)  #FJK
